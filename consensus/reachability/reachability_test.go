...
func TestAddChild(t *testing.T) {
	...
	lastChild := newReachabilityTreeNode(&blocknode.BlockNode{})
	modifiedNodes := newModifiedTreeNodes()
	...
	for i := 0; i < len(childNodes); i++ {
		childNodes[i] = newReachabilityTreeNode(&blocknode.BlockNode{})
		modifiedNodes := newModifiedTreeNodes()
...
func TestReindexIntervalErrors(t *testing.T) {
	...
	treeNode := newReachabilityTreeNode(&blocknode.BlockNode{})
	...
	for i := 0; i < 100; i++ {
		childTreeNode := newReachabilityTreeNode(&blocknode.BlockNode{})
...
func BenchmarkReindexInterval(b *testing.B) {
	for i := 0; i < b.N; i++ {
		...
		fullReindexTriggeringNode := newReachabilityTreeNode(&blocknode.BlockNode{})
		b.StartTimer()
...
func TestFutureCoveringTreeNodeSetString(t *testing.T) {
	treeNodeA := newReachabilityTreeNode(&blocknode.BlockNode{})
	treeNodeA.interval = newReachabilityInterval(123, 456)
	treeNodeB := newReachabilityTreeNode(&blocknode.BlockNode{})
...
func TestReachabilityTreeNodeString(t *testing.T) {
	...
	treeNodeB1 := newReachabilityTreeNode(&blocknode.BlockNode{})
	treeNodeB1.interval = newReachabilityInterval(100, 150)
	treeNodeB2 := newReachabilityTreeNode(&blocknode.BlockNode{})
	treeNodeB2.interval = newReachabilityInterval(150, 199)
	treeNodeC := newReachabilityTreeNode(&blocknode.BlockNode{})
...
//	dag, teardownFunc, err := blockdag.DAGSetup("TestIsInPast", true, blockdag.Config{
...
//	blockA := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{dag.genesis.Hash()}, nil)
//	blockB := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{blockA.BlockHash()}, nil)
//
//	// Add another block above the genesis
//	blockC := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{dag.genesis.Hash()}, nil)
...
//	blockD := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{blockB.BlockHash(), blockC.BlockHash()}, nil)
//	nodeD, ok := dag.blockNodeStore.LookupNode(blockD.BlockHash())
//	if !ok {
//		t.Fatalf("TestIsInPast: block C is not in the block index")
//	}
//
...
//	// Create a new database and DAG instance to run tests against.
//	dag, teardownFunc, err := blockdag.DAGSetup("TestAddChildThatPointsDirectlyToTheSelectedParentChainBelowReindexRoot",
//		true, blockdag.Config{DAGParams: &dagconfig.SimnetParams})
//	if err != nil {
...
//	chainRootBlock := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{dag.genesis.Hash()}, nil)
//
...
//		chainBlock := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{chainRootBlockTipHash}, nil)
//		chainRootBlockTipHash = chainBlock.BlockHash()
//	}
//
//	// Add another block over genesis
//	blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{dag.genesis.Hash()}, nil)
//}
//
//func TestUpdateReindexRoot(t *testing.T) {
//	// Create a new database and DAG instance to run tests against.
//	dag, teardownFunc, err := blockdag.DAGSetup("TestUpdateReindexRoot", true, blockdag.Config{
//		DAGParams: &dagconfig.SimnetParams,
//	})
//	if err != nil {
//		t.Fatalf("Failed to setup DAG instance: %v", err)
...
//	chain1RootBlock := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{dag.genesis.Hash()}, nil)
//	chain2RootBlock := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{dag.genesis.Hash()}, nil)
//
...
//		chain1Block := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{chain1RootBlockTipHash}, nil)
//		chain1RootBlockTipHash = chain1Block.BlockHash()
//
//		chain2Block := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{chain2RootBlockTipHash}, nil)
...
//	blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{chain1RootBlockTipHash}, nil)
//
...
//	// Create a new database and DAG instance to run tests against.
//	dag, teardownFunc, err := blockdag.DAGSetup("TestReindexIntervalsEarlierThanReindexRoot", true, blockdag.Config{
//		DAGParams: &dagconfig.SimnetParams,
//	})
//	if err != nil {
//		t.Fatalf("Failed to setup DAG instance: %v", err)
...
//	leftBlock := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{dag.genesis.Hash()}, nil)
//	centerBlock := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{dag.genesis.Hash()}, nil)
//	rightBlock := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{dag.genesis.Hash()}, nil)
//
...
//	for i := uint64(0); i < reachabilityReindexWindow; i++ {
//		block := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{centerTipHash}, nil)
...
//	for i := uint64(0); i < reachabilityReindexWindow-1; i++ {
//		block := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{leftTipHash}, nil)
...
//	for i := uint64(0); i < reachabilityReindexWindow-1; i++ {
//		block := blockdag.PrepareAndProcessBlockForTest(t, dag, []*daghash.Hash{rightTipHash}, nil)
