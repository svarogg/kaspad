...
type thresholdConditionChecker interface {
	...
	// needed.
	Condition(*blocknode.BlockNode) (bool, error)
}
...
func (dag *BlockDAG) thresholdState(prevNode *blocknode.BlockNode, checker thresholdConditionChecker, cache *thresholdStateCache) (ThresholdState, error) {
	...
	var neededStates []*blocknode.BlockNode
	for prevNode != nil {
		...
		medianTime := dag.PastMedianTime(prevNode)
...
//
// This function MUST be called with the DAG state lock held (for writes).
func (dag *BlockDAG) deploymentState(prevNode *blocknode.BlockNode, deploymentID uint32) (ThresholdState, error) {
