...
func TestAcceptingInInit(t *testing.T) {
	...
	// Load the test block
	blocks, err := test.LoadBlocks("../test/blk_0_to_4.dat")
	if err != nil {
		t.Fatalf("Error loading file: %v\n", err)
	}
...
//	currentDifference := utxodiffstore.maxBlueScoreDifferenceToKeepLoaded
//	utxodiffstore.maxBlueScoreDifferenceToKeepLoaded = math.MaxUint64
//	defer func() { utxodiffstore.maxBlueScoreDifferenceToKeepLoaded = currentDifference }()
//
...
func TestDAGIndexFailedStatus(t *testing.T) {
	...
	if invalidBlockNode.Status()&blocknode.StatusValidateFailed != blocknode.StatusValidateFailed {
	...
	if invalidBlockChildNode.Status()&blocknode.StatusInvalidAncestor != blocknode.StatusInvalidAncestor {
	...
	if invalidBlockGrandChildNode.Status()&blocknode.StatusInvalidAncestor != blocknode.StatusInvalidAncestor {
...
func TestPastUTXOMultiSet(t *testing.T) {
	...
	blockCSelectedParentMultiset, err := dag.multiSetManager.SelectedParentMultiset(blockNodeC)
	if err != nil {
	...
	blockCSelectedParentMultiSetAfterAnotherBlock, err := dag.multiSetManager.SelectedParentMultiset(blockNodeC)
	if err != nil {
		t.Fatalf("TestPastUTXOMultiSet: selectedParentMultiset unexpectedly failed: %s", err)
	}
