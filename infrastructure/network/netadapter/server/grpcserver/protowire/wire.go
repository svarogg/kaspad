...
func toRPCPayload(message appmessage.Message) (isKaspadMessage_Payload, error) {
	...
	case *appmessage.GetMempoolEntriesResponseMessage:
		...
		return payload, nil
	case *appmessage.GetVersionRequestMessage:
		payload := new(KaspadMessage_GetVersionRequest)
		err := payload.fromAppMessage(message)
		if err != nil {
			return nil, err
		}
		return payload, nil
	case *appmessage.GetVersionResponseMessage:
		payload := new(KaspadMessage_GetVersionResponse)
		err := payload.fromAppMessage(message)
		if err != nil {
			return nil, err
		}
